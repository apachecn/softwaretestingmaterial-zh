...
****[![framework structure](../Images/f3ee8d32dc987c5d46811ce4e07ff969.png)](https://www.softwaretestingmaterial.com/wp-content/uploads/2017/07/framework-structure.png) 上面的截图说明了一个标准化的 maven 项目。按照上面的 maven 项目，所有的测试都保存在' *** src/test/java ** '* 和剩余文件(比如 config.properties，element locators (POM 类)，utility 文件，测试数据等)中。，)保存在' *** src/main/java ** '* 下。****
